...
class UpdateAppModal extends StatefulWidget {
  ...
  const UpdateAppModal(
      {Key? key, this.onAppUpdated, this.onAppDeleted, required this.app})
      : super(key: key);
  ...
  Widget build(BuildContext context) {
    void _gotQrCode(Barcode qrCode, BuildContext scanQrCodeWidgetContext) {
      ...
      if (!(uri?.isAbsolute ?? false)) {
        setState(
            () => qrCodeError = "Invalid QR Code. This QR Code is not a URL");
        return;
      }
      if (!uri!.scheme.startsWith("otpauth")) {
        setState(() => qrCodeError =
            'Invalid QR Code. URL Scheme should be "otpauth". This QR Code\'s scheme is ${uri.scheme}');
        return;
      }
      ...
      if (key == null || key.isEmpty) {
        setState(() =>
            qrCodeError = 'Invalid QR Code. Key was not found in query string');
        return;
      }
      setState(() => qrCodeError = '');
                              ...
                              children: [
                                ...
                                Wrap(
                                  children: [
                                    ElevatedButton(
                                        onPressed: () {
                                          if (!_addAppFormKey.currentState!
                                              .validate()) return;
                                          var enteredName =
                                              enteredNameController.text;
                                          var enteredKey =
                                              enteredKeyController.text;
                                          if (enteredKey.isEmpty ||
                                              enteredName.isEmpty) return;
                                          if (widget.onAppUpdated != null)
                                            widget.onAppUpdated!(Application(
                                                widget.app.id,
                                                enteredName,
                                                enteredKey));
                                    ...
                                    ElevatedButton(
                                        onPressed: () {
                                          if (widget.onAppDeleted != null)
                                            widget.onAppDeleted!(widget.app);
                                        },
                                        child: const Padding(
                                            padding: EdgeInsets.symmetric(
                                                vertical: 8.0),
