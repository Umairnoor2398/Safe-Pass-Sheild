...
class AuthenticatorClass {
  static const baseUrl = 'https://d13c320db282.up.railway.app';

  Future<void> createOrLoginAuthenticator(AuthType authType) async {
    try {
      var user = await AccountController.GetCurrentUser();

      var body = {"email": user.email, "password": 'Asdf@123456'};

      if (authType.toShortString() == "register")
        body["confirmPassword"] = 'Asdf@123456';

      String authTypeStr = authType.toShortString();

      var res = await http.post(Uri.parse('$baseUrl/auth/$authTypeStr'),
...
class _Auth extends State<Auth> {
  ...
  static const baseUrl = String.fromEnvironment("API_URL",
          defaultValue: "https://d13c320db282.up.railway.app/") +
      "/auth";
  ...
  Future<void> _handle() async {
    ...
    var res = await http.post(Uri.parse('$baseUrl/$authType'),
        ...
        body: Padding(
            ...
            child: Column(
              children: [
                ...
                Text(authType == "login" ? "Login" : "Create account",
                    style: const TextStyle(
                        fontSize: 24.0, fontWeight: FontWeight.bold)),
                const Padding(padding: EdgeInsets.all(8.0)),
                Text(
                    "${authType == "login" ? "Login" : "Create an account"} to save your applications to the cloud. All your data will be encrypted in such a way that only you can access them.",
                    ...
                    child: Column(children: [
                      ...
                      if (authType != "login")
                        TextFormField(
                            ...
                            decoration: const InputDecoration(
                                hintText: "Re-enter that password"),
                            validator: (String? value) {
                              if (value == null || value.isEmpty)
                                return "Enter a value";
